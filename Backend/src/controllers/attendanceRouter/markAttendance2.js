...
// CONFIGURATION
const QR_SECRET_KEY = "PARIKSHA_LIBRARY_PRO_QR_2026"; 

const markAttendance2 = async (req, res) => {
    try {
        const { userId, qrData } = req.body;

        // 1. Verify QR Data
        if (qrData !== QR_SECRET_KEY) {
            return res.status(401).json({
                message: "Invalid QR Code. Please scan the official library QR.",
                status: "failed"
            });
        }
...
module.exports = markAttendance2;